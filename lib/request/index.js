...
function Request (method, args, attempts, proxyCondition) {
    ...
    if (!args.options.hasOwnProperty('followLocation')) {
        args.options.followLocation = true;
    }
    if (args.options.proxy == false) {
        args.proxy = null;
    } else {
        if (proxyCondition && !proxy.isActive()) {
    ...
    if (!args.options.async) { // Synchronous
        ...
        if (resp.err) {
            ...
            if ([301, 302].indexOf(resp.status) != -1) {
                if (args.options.followLocation) {
                    let location = url.parse(resp.headers.location);
                    if (!location.protocol) {
                        let srcUrl = url.parse(args.url);
                        // add protocol for location without proto, example "//smappi.org/path/to" or relative path "/path/to"
                        location.protocol = srcUrl.protocol;
                        if (!location.href.startsWith('//')) { // relative, not shortcut proto
                            // relative path to current hostname, example "/path/to"
                            location.hostname = srcUrl.hostname;
                        }
                    }
                    args.url = location.format();
                    return new Request(method, args, attempts + 1, proxyCondition);
                }
            }
