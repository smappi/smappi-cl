...
function next (options) {
    ...
    if (_PROXIES.filter(p => !p.skip).length == 0) { // if proxies is all skipped
    ...
    for (; idx < _PROXIES.length; idx++) {
        // console.log('idx', idx)
        if (_PROXIES[_PROXIDX] === undefined) break;
        if (idx == _PROXIDX || _PROXIES[idx].skip) continue;
        if (_PROXIES[idx].cnt < _PROXIES[_PROXIDX].cnt) break;
    }
    // console.log('_PROXIDX old', _PROXIDX)
    _PROXIDX = idx; // new proxy idx
    // console.log('_PROXIDX new', _PROXIDX)
