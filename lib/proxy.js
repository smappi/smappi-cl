...
function Proxy () {
    ...
    this.currentHref = function () {
        ...
        if (proxy) {
            return this.buildHref(proxy);
        }
        return undefined;
    }

    /**
     * Build Href by proxy object
     *
     * @return String - Proxy Href
     */
    this.buildHref = function (proxy) {
    ...
    this.next = function (options) {
        ...
        for (var key in this.proxies) {
            ...
            else if (current === {}) break;  // use first key as current (below)
            else if (key === this.currentKey || this.proxies[key].skip) continue; // search next
            else if (this.proxies[key].cnt < current.cnt) break; // use proxy with a smaller counter of using
            else break;
        }
        this.currentKey = key; // new current proxy
